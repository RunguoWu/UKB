...
gen_next_cycle <- function(prim_flag, j, v_j, 
                           ...
                           scaleby, intercept_calibrated_i,
                           use_cost = FALSE, 
                           side_effects, had_mvevd, # add mvevd: RW:2021-06-15
                           had_rhabdo, had_myop) {
  ...
  #     v_j[[paste0(int_names_noage[[i]][1],"_noageint_", int_names_noage[[i]][2])]] <- 
  ...
  if (nonlinage) {
    ...
    for (v_int in int_names_b_nonlinage){
      if (grepl("dm_", v_int)) v_b_int_nonlinage[v_int] <- v_j[v_int] # RW 210827: add dm
      for (gp in 1:2)
  ...
  # output side effects and treatment effect to check
  # sf <-  c(or_dm_low,or_dm_high,p_myop, p_rhabdo, p_nvd_rhabdo)
  # saveRDS(sf, paste0("Z:/PCTU/HEALTH ECONOMICS/CVD_HE/UKB/output/vali_output/", "sf", j, ".rds"))
  
  # saveRDS(lam_b_i, paste0("Z:/PCTU/HEALTH ECONOMICS/CVD_HE/UKB/output/vali_output/", "lam", j, ".rds"))
  ...
  for (e in events_nf_rand) {
    ...
    # TODO: if !is.na(t_since_e_i[[e]]), don't run p_event 
    ...
    if (p_e > p_rand_e) {
      ...
      v_j[varname_0_1] <- 1
      # RW: 21-09-27 if incident diabetes happens, recode no diabetes (dmPre) 0
  ...
  if (had_mvevd == 0){
    ...
    if (v_j["mi_0_1"]==1 | v_j["stroke_0_1"] ==1 
      ...
      had_mvevd <- 1 # add mvevd: RW:2021-06-15
    }
  }
  ...
  ret_list <- list(v_j = v_j, 
                   ...
                   had_mvevd = had_mvevd) # add mvevd: RW:2021-06-15
...
master <- function(.input_data_dir, 
                   ...
                   sample_nodm,
                   dm_predict,
                   id_list # for select individuals
){
  ...
  if (!identical(tx, FALSE)) {
    ...
    # # TODO: test, force no side effects on dm
    # side_effects$or_dm_low <- 1
    # side_effects$or_dm_high <- 1
    
    # side_effects$p_myop <- side_effects$p_myop * sens_multiplier
    # side_effects$p_rhabdo <- side_effects$p_rhabdo * sens_multiplier
    # side_effects$or_dm_low <- (side_effects$or_dm_low - 1) * sens_multiplier + 1
    # side_effects$or_dm_high <- (side_effects$or_dm_high - 1) * sens_multiplier + 1
  }
  ...
  # QoL with event coefficent * 0.5
  # if (sens_multiplier == 2) { # just for temporary use, mute this later
  #   # borrow sens_multiplier as the switch of qol * 0.5
  #   
  #   tx_tag <- paste0(tx_tag, "qol05")
  #   
  #   # only change cf_t, with two age spline unchanged
  #   cf_t_temp <- cf_temp[["qol"]][["cf_t"]]
  #   unchg <- cf_t_temp[c("CurrAge_cent_1", "CurrAge_cent_2")]
  #   chg <- cf_t_temp[!grepl("CurrAge_cent", names(cf_t_temp))]
  #   chg <- chg * 0.5
  #   cf_t[["qol"]] <- c(unchg, chg)
  #   
  #   if (prim_flag == "sec") {
  #     # cvd history in cf_b need change as well
  #     
  #     cf_b_unchg <- cf_b[!grepl("only|number", names(cf_b))] # for cvd histories
  #     cf_b_chg <- cf_b[grepl("only|number", names(cf_b))]
  #     cf_b_chg <- cf_b_chg * 0.5
  #     cf_b <- c(cf_b_unchg, cf_b_chg)
  #     lam_b[["qol"]] <- mx_b[, names(cf_b)] %*% cf_b
  #   }
  # }
  ...
  # life year, mvevd, so +2 further: RW:2021-06-15

  if (nonlinage) v_output_null <- matrix(nrow = 0, ncol = N + 4 +2) else
    v_output_null <- matrix(nrow = 0, ncol = N + 2 +2)
  ...
  # life year: RW:2021-06-15
  vars_to_summarise <- c(vars_to_summarise, "ly")
  # mvevd: RW:2021-06-15
  vars_to_summarise <- c(vars_to_summarise, "mvevd")
  ...
  if (id_list == TRUE)
    ids <- readRDS(file.path(.input_data_dir, str_c("id_list_",prim_flag, ".rds")))[,1]
  
  # # for switch prediction of icd dm to off for people without base dm 
  # if (sample_nodm == TRUE) {
  #   
  #   ids <- mx_t %>% as.data.frame() %>% 
  #     filter(dm_0_10 ==0 & dm_10_inf==0) %>%
  #     select(id_new) %>%
  #     sample_n(800) %>% 
  #     arrange(id_new)
  #   
  #   ids <- ids$id_new
  #   
  #   # force to not predict dm if dm_predict==0
  #   events_to_predict[ids, "dm"] <- dm_predict
  # }
  # 
  # export LDL reduction if using selection of ids
  if (length(ids) < nrow(mx_b)){
    output_ldl <- cbind(mx_b[ids, c("id_new", "ldl_red")], age = mx_t[ids, "CurrAge_cent"]*10+60)
    saveRDS(output_ldl,  file = file.path(output_dir, str_c(prim_flag, "ldl_red.rds")))
  }
  ...
  retval <- foreach (i = ids) %dopar% {
    ...
    for (n in 1 : n_sim) {
      ...
      # life year: RW:2021-06-15
      ly <- cbind("ly"=rep(NA, J))
      mx_output_null <- cbind(mx_output_null, ly)
      
      # mvevd: RW:2021-06-15
      mvevd <- cbind("mvevd"=rep(NA, J))
      mx_output_null <- cbind(mx_output_null, mvevd)
      ...
      # life year and mvevd: RW:2021-06-15: insert ly and mvevd
      ...
      had_rhabdo <- 0
      had_myop <- 0
      had_mvevd <- 0 # add mvevd: RW:2021-06-15
      ...
      for (j in 1:J) {
        ...
        had_mvevd <- alpha$had_mvevd # add mvevd: RW:2021-06-15
  ...
  # dm_predict_tag <- if (dm_predict==0) "dmpre0" else ""
