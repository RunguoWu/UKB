...
# use one parametre list file to contain multiple files
pf <- list()

# read all variables in cf file -------------------------------------------

cf <- readRDS(file.path(vali_data, "cf_20210714.rds"))
# the old cf without dm is enough, as no new variable join for dm
...
# NOTE: set the cut-off date to 2020-12-31 for this part!
...
.ctt_var <- c("Intercept", "male", "smoker","race_afro", "race_otherna", "unhealthy_diet",  
             "cvd_only", "h_mi_raw_only", "h_pad_raw_only", "othchd_only", "number_events2", 
             "lnbcreann", "dias_bpS", "sys_bpS", "lhdl", "NEWB_LDL_CL_cent", 
             "LDL_nostatin_cent", "hba1c", "dmT1","revised_regimen", 
...
ukb_base_prim <- ukb_b_prim %>% 
  mutate(statin = if_else(revised_regimen=="none", 0, 1)) %>% 
  select("id_new", "male", "over_65", "RG5", "dm", "CAN", "dmT1", "statin") %>% 
  rename("dm_baseline" = "dm")

ukb_base_sec <- ukb_b_sec %>% 
  mutate(statin = if_else(revised_regimen=="none", 0, 1)) %>%
  select("id_new", "male", "over_65", "RG5", "dm", "CAN", "dmT1", "statin", "othchd_only") %>% 
  rename("dm_baseline" = "dm")

ukb_base_prim$RG5 <- as.character(ukb_base_prim$RG5)
...
ukb_base_sec$RG5 <- as.character(ukb_base_sec$RG5)
...
for (prim in c("prim", "sec")) {
  ...
  pf[["events_to_predict"]][[prim]] <- events_to_predict
}


# delete old or useless vars.
ukb_b_prim <- ukb_b_prim %>% 
  select(-over_65, -RG5, -CAN, -dm, -male_int_dm, -smoker, -revised_regimen)
ukb_b_sec <- ukb_b_sec %>% 
  select(-over_65, -RG5, -CAN, -dm, -male_int_dm, -smoker, -revised_regimen)
...
ukb_t$CurrAge_cent_int_smk_ex <- ukb_t$CurrAge_cent*d4s$smk_ex
ukb_t$CurrAge_cent_int_male <- ukb_t$CurrAge_cent*d4s$male
ukb_t$CurrAge_cent_int_hba1c <- ukb_t$CurrAge_cent*d4s$hba1c
...
# RW: new dm variable 2021-09-27

# ukb_t$dm_10_20 <- ukb_t$dm_20_inf <- 0
# 
# ukb_t[["dm_10_20"]][d4s[["diabetes.fo.pre"]]==1 & 
#                      d4s[["diabetes.fo.pre.date"]] + 6939.75 > d4s$recruit.date &
#                       d4s[["diabetes.fo.pre.date"]] + 3652.5 <= d4s$recruit.date + 365.25] <- 1
# 
# ukb_t[["dm_20_inf"]][d4s[["diabetes.fo.pre"]]==1 &
#                        d4s[["diabetes.fo.pre.date"]] + 7305 <= d4s$recruit.date + 365.25] <- 1

ukb_t$dmPre_1 <- ukb_t$dmPre_3 <- ukb_t$dmPre_4 <- 0

ukb_t[["dmPre_1"]][ukb_t[["dm_0_10"]]==0 & ukb_t[["dm_10_inf"]]==0 & 
                  d4s[["hba1c"]]<32] <- 1

# ukb_t[["dmPre_2"]][ukb_t[["dm_0_10"]]==0 & ukb_t[["dm_10_inf"]]==0 & 
#                      d4s[["hba1c"]]>=32 & d4s[["hba1c"]]<37] <- 1

ukb_t[["dmPre_3"]][ukb_t[["dm_0_10"]]==0 & ukb_t[["dm_10_inf"]]==0 & 
                     d4s[["hba1c"]]>=37 & d4s[["hba1c"]]<42] <- 1

ukb_t[["dmPre_4"]][ukb_t[["dm_0_10"]]==0 & ukb_t[["dm_10_inf"]]==0 & 
                     d4s[["hba1c"]]>=42] <- 1


ukb_t$CurrAge_cent_int_dm_0_10 <- ukb_t$CurrAge_cent*ukb_t$dm_0_10
ukb_t$CurrAge_cent_int_dm_10_inf <- ukb_t$CurrAge_cent*ukb_t$dm_10_inf

ukb_t$mi_0_1_noageint_dm_0_10 <- ukb_t$dm_0_10*ukb_t$mi_0_1
ukb_t$mi_0_1_noageint_dm_10_inf <- ukb_t$dm_10_inf*ukb_t$mi_0_1
ukb_t$stroke_0_1_noageint_dm_0_10 <- ukb_t$dm_0_10*ukb_t$stroke_0_1
ukb_t$stroke_0_1_noageint_dm_10_inf <- ukb_t$dm_10_inf*ukb_t$stroke_0_1
# 
# ukb_t$lnbcreann_noageint_dm_0_10 <- ukb_t$dm_0_10*d4s$lnbcreann
# ukb_t$lnbcreann_noageint_dm_10_inf <- ukb_t$dm_10_inf*d4s$lnbcreann
...
ukb_t$t_cancer[d4s[["cancer.baseline.all"]]==1 & 
                 d4s[["cancer.baseline.all.date"]] + 365.25*21 <= d4s$recruit.date + 365.25] <- 21
...
# }
# ukb_t$t_dm[d4s[["diabetes.fo.pre"]]==1 & 
#            d4s[["diabetes.fo.pre.date"]] + 365.25*11 <= d4s$recruit.date + 365.25] <- 11
...
ukb_t$t_dm[d4s[["diabetes.fo.pre"]]==1 & 
          d4s[["diabetes.fo.pre.date"]] + 365.25*21 <= d4s$recruit.date + 365.25] <- 21
...
# save to one parameter file ---------------------------------------------

saveRDS(pf, file =  file.path(vali_data, "pf.rds"), compress = F)
